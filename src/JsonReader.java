...
public class JsonReader {
    ...
    public JsonReader(String data) throws FileNotFoundException {
        reader = new TokenReader(new BufferedReader(new InputStreamReader(new ByteInputStream(data.getBytes(),data.length()))));
    }
            ...
            switch (currentToken) {
                case BOOLEAN:
                    ...
                    if (hasStatus(STATUS_EXPECT_OBJECT_VALUE)) {
                        Boolean bool = reader.readBoolean();
                        String key = stack.pop().valueAsKey();
                        stack.peek().valueAsObject().put(key, bool);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_OBJECT;
                        continue;
                    }
                    if (hasStatus(STATUS_EXPECT_ARRAY_VALUE)) {
                        Boolean bool = reader.readBoolean();
                        stack.peek().valueAsArray().add(bool);
                ...
                case NULL:
                    ...
                    if (hasStatus(STATUS_EXPECT_OBJECT_VALUE)) {
                        reader.readNull();
                        String key = stack.pop().valueAsKey();
                        stack.peek().valueAsObject().put(key, null);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_OBJECT;
                        continue;
                    }
                    if (hasStatus(STATUS_EXPECT_ARRAY_VALUE)) {
                        reader.readNull();
                        stack.peek().valueAsArray().add(null);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_ARRAY;
                        continue;
                    }
                ...
                case NUMBER:
                    ...
                    if (hasStatus(STATUS_EXPECT_OBJECT_VALUE)) {
                        Number number = reader.readNumber();
                        String key = stack.pop().valueAsKey();
                        stack.peek().valueAsObject().put(key, number);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_OBJECT;
                        continue;
                    }
                    if (hasStatus(STATUS_EXPECT_ARRAY_VALUE)) {
                        Number number = reader.readNumber();
                        stack.peek().valueAsArray().add(number);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_ARRAY;
                        continue;
                    }
                ...
                case STRING:
                    ...
                    if (hasStatus(STATUS_EXPECT_OBJECT_VALUE)) {
                        String str = reader.readString();
                        String key = stack.pop().valueAsKey();
                        stack.peek().valueAsObject().put(key, str);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_OBJECT;
                        continue;
                    }
                    if (hasStatus(STATUS_EXPECT_ARRAY_VALUE)) {
                        String str = reader.readString();
                        stack.peek().valueAsArray().add(str);
                        status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_ARRAY;
                        continue;
                    }
                ...
                case END_ARRAY:
                    if (hasStatus(STATUS_EXPECT_END_ARRAY)) {
                        ...
                        int type = stack.peek().type;
                        if (type == StackValue.TYPE_OBJECT_KEY) {
                            // key: [ CURRENT ] ,}
                            String key = stack.pop().valueAsKey();
                            stack.peek().valueAsObject().put(key, array.valuse);
                            status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_OBJECT;
                            continue;
                        }
                        if (type == StackValue.TYPE_ARRAY) {
                            // xx, xx, [CURRENT] ,]
                            stack.peek().valueAsArray().add(array.valuse);
                            status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_ARRAY;
                            continue;
                        }
                    }
                ...
                case END_OBJECT:
                    if (hasStatus(STATUS_EXPECT_END_OBJECT)) {
                        ...
                        if (type == StackValue.TYPE_OBJECT_KEY) {
                            String key = stack.pop().valueAsKey();
                            stack.peek().valueAsObject().put(key, object.valuse);
                            status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_OBJECT;
                            continue;
                        }
                        if (type == StackValue.TYPE_ARRAY) {
                            stack.peek().valueAsArray().add(object.valuse);
                            status = STATUS_EXPECT_COMMA | STATUS_EXPECT_END_ARRAY;
                            continue;
                        }
                    }
                ...
                case BEGIN_ARRAY:
                    if (hasStatus(STATUS_EXPECT_BEGIN_ARRAY)) {
                        stack.push(StackValue.newJsonArray(new ArrayList<>()));
